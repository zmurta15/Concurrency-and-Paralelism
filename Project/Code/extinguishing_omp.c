	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( step=0; step<10; step++ )	{
			...
			#pragma omp parallel 
			{
				...
				for( i=1; i<rows-1; i++ )
					for(j=1; j<columns-1; j++ ) 
						if ( fabs( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) ) > my_max ) {
							my_max = fabs( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) );
						}
				#pragma omp critical
				{
					if (my_max > global_residual)
						global_residual = my_max;
				}
		}
		...
		#pragma omp parallel 
		{
			...
			for( t=0; t<num_teams; t++ ) {
				...
				if ( teams[t].target == -1 ) continue; 

				/* 4.3.4. Move in the focal point direction */
				if ( teams[t].type == 1 ) { 
					// Type 1: Can move in diagonal
					if ( focal[teams[t].target].x < teams[t].x ) teams[t].x--;
					if ( focal[teams[t].target].x > teams[t].x ) teams[t].x++;
					if ( focal[teams[t].target].y < teams[t].y ) teams[t].y--;
					if ( focal[teams[t].target].y > teams[t].y ) teams[t].y++;
				}
				else if ( teams[t].type == 2 ) { 
					// Type 2: First in horizontal direction, then in vertical direction
					if ( focal[teams[t].target].y < teams[t].y ) teams[t].y--;
					else if ( focal[teams[t].target].y > teams[t].y ) teams[t].y++;
					else if ( focal[teams[t].target].x < teams[t].x ) teams[t].x--;
					else if ( focal[teams[t].target].x > teams[t].x ) teams[t].x++;
				}
				else {
